...
//        UpdateLeds(0x0000);
    ...
    else if(but1 > 0 && but1 <= 1024)
    {
        LED_CONTROL_WriteCompare(16383);
        //UpdateLeds(0xFFFF);
        UpdateSwitchLeds(0x0000);
        data.pos = 2;
        data.posData = 75;
        i2cReadBuffer[0] = data.pos;
        i2cReadBuffer[1] = data.posData;
...
void UpdateSwitchLeds(uint16 curPos)
{
...
void UpdateLeds(uint16 curPos)
{
    ...
    LED4_Write(0);
    LED5_Write(0);
    UART_UartPutString("in UpdateLeds\n");
       
    if (curPos > 10 || curPos <= 10)
    {
        UART_UartPutString("in UpdateLeds :: range 0-10\n");
        LED1_Write(1);
    }
    
    if (curPos > 20)
    {
        UART_UartPutString("in UpdateLeds :: range >20\n");
        LED2_Write(1);
    }
    
    if (curPos > 40)
    {
        UART_UartPutString("in UpdateLeds :: range >40\n");
        LED3_Write(1);
    }
    
    if (curPos > 60)
    {
        UART_UartPutString("in UpdateLeds :: range > 60\n");
        LED4_Write(1);
    }
    
    if (curPos > 80)
    {
        UART_UartPutString("in UpdateLeds :: range > 80\n");
        LED5_Write(1);
    }
}
